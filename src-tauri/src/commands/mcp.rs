...
/// Helper to execute claude plugin command
fn execute_claude_plugin_command(app_handle: &AppHandle, args: Vec<&str>) -> Result<String> {
    info!("Executing claude plugin command with args: {:?}", args);
    let claude_path = find_claude_binary(app_handle)?;
    let mut cmd = create_command_with_env(&claude_path);
    cmd.arg("plugin");
    for arg in args {
...
#[tauri::command]
pub async fn plugin_install(app: AppHandle, name: String) -> Result<String, String> {
    info!("Installing plugin: {}", name);
    execute_claude_plugin_command(&app, vec!["install", &name])
        .map(|output| output.trim().to_string())
        .map_err(|e| e.to_string())
}

/// Uninstalls a Claude Code plugin
#[tauri::command]
pub async fn plugin_uninstall(app: AppHandle, name: String) -> Result<String, String> {
    info!("Uninstalling plugin: {}", name);
    execute_claude_plugin_command(&app, vec!["uninstall", &name])
        .map(|output| output.trim().to_string())
        .map_err(|e| e.to_string())
}
