...
/// Execute the Claude Agent SDK via a Node.js script (Sidecar pattern)
/// Execute the Claude Agent SDK via a Node.js script (Sidecar pattern)
#[tauri::command]
pub async fn execute_sdk_agent(
    app: AppHandle,
    project_path: String,
    prompt: String,
    model: String,
    ...
    log::info!(
        "Starting SDK Agent in: {} with model: {} (session: {:?})",
        project_path,
        model,
        session_id
    );
    ...
    // We assume the app is running from the project root in dev mode
    let cwd = std::env::current_dir().map_err(|e| e.to_string())?;
    // Navigate to the script location relative to project root
    let script_path = cwd.join("src-tauri/scripts/run-agent.ts");

    if !script_path.exists() {
        return Err(format!("Agent script not found at: {:?}", script_path));
    }

    // We use 'bun' as the runner since the project uses bun
    let runner = "bun";
    ...
    // Reuse the existing spawn logic which handles streaming output mapping
    spawn_claude_process(app, cmd, prompt, model, project_path).await
}
