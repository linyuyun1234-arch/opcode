...
import { Bot, Loader2, Play, Clock, CheckCircle, XCircle, Trash2, Import, ChevronDown, ChevronRight, FileJson, Globe, Download, Plus, History, Edit, Store } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const Agents: React.FC = () => {
  ...
  // Agent Market States
  const [templates, setTemplates] = useState<{name:string, description:string, prompt:string, category:string}[]>([]);
  ...
  const loadTemplates = async () => {
    try {
      ...
      const t = await api.fetchAgentTemplates();
      setTemplates(t);
    } catch (e) {
  ...
  const handleUseTemplate = (t: typeof templates[0]) => {
      // Set as editing agent but w/o ID to trigger creation
      setEditingAgent({
          name: t.name,
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid grid-cols-3 w-full max-w-md mb-6 h-auto p-1">
              ...
              <TabsTrigger value="store" className="py-2.5 px-3">
                  ...
                  {agents.map((agent) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground mb-3 line-clamp-2 min-h-[2.5rem]">
                        {agent.system_prompt ? agent.system_prompt.substring(0, 100) + "..." : "No prompt provided"}
                      </p>
                ...
                {loadingTemplates ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {templates.map((t, idx) => (
                            ...
                            >
                                <Card className="p-5 hover:shadow-lg transition-all h-full flex flex-col cursor-pointer border-dashed border-2 hover:border-solid hover:border-primary group" onClick={() => handleUseTemplate(t)}>
