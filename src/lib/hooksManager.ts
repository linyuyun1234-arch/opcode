      ...
      for (const matcher of matchers) {
        ...
        if (matcher.matcher) {
          try {
            ...
            // Note: Safari/WebKit may throw "Invalid regular expression: invalid group specifier name"
            // for lookbehind assertions or named groups
            new RegExp(matcher.matcher);
          } catch (e) {
            // Silently skip invalid regex patterns instead of blocking the entire config
            // This is a workaround for Safari/WebKit compatibility issues
            console.warn(`[HooksManager] Skipping invalid regex pattern: ${matcher.matcher}`, e);
            errors.push({
              event,
